...
const Item = ({ icon, label, to, expanded }: SidebarItem & { expanded: boolean }) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
      `flex items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-[hsl(var(--secondary))] ${isActive ? "bg-milk text-black" : "text-zinc-400"}`
    }
  >
      ...
      <nav className="flex flex-col gap-1">
        ...
        {user && (
          <button
            ...
            className="flex items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-[hsl(var(--secondary))] text-zinc-400"
          >
